...
**场景 A：老用户（有 config + 有 OKR 候选）** — 一次弹出 4 个问题：

| 问题 | header | 选项 |
|------|--------|------|
| 选择日期范围？ | 日期范围 | 近 14 天 / 本周 / 上周 |
| 选择项目范围？（仓库：{repo_root}） | 项目范围 | 扫描全部 / Other 输入项目名 |
| 检测到你的 OKR 候选，是否使用？ | OKR | 使用推荐 OKR / 选择其他 OKR（Other）/ 更新（Other 输入新内容覆盖）/ 跳过 |
| 是否有补充工作内容？ | 补充 | 无补充 / 有补充（Other 输入） |

**场景 B：新用户（无 config 或无 OKR 候选）** — 缺什么问什么，合并到一次交互中：
...
|--------|------|
| `{用户名}-okr.md` | 默认当前 OKR |
| `{用户名}-okr-current.md` | 显式当前 OKR，优先级高于默认文件 |
| `{用户名}-okr-2026h1.md` | 2026 上半年 OKR |
| `{用户名}-okr-2026h2.md` | 2026 下半年 OKR |
| `{用户名}-okr-review-2025h2.md` | 历史复盘 / 回顾材料，不作为周报默认 OKR |

OKR 候选发现规则：

1. 扫描 `$OKR_DIR/{用户名}-okr*.md`。
2. 用户消息中明确指定 OKR 文件名或路径时，优先使用用户指定文件。
3. 存在 `{用户名}-okr-current.md` 时，默认推荐它。
4. 否则存在 `{用户名}-okr.md` 时，默认推荐它。
5. 否则按周报日期范围匹配周期文件：`YYYYh1` 覆盖 1-6 月，`YYYYh2` 覆盖 7-12 月。
6. 文件名包含 `review`、`复盘`、`回顾` 的 OKR 文件只作为历史参考，不自动作为周报 KR 对齐依据。
7. 如果有多个可用候选且无法唯一确定，展示文件列表和每个文件前几行摘要，让用户选择。

检测到多个候选时，询问：

| 问题 | header | 选项 |
|------|--------|------|
| 本次周报使用哪个 OKR？ | OKR | 使用推荐 OKR / 选择其他 OKR（Other）/ 跳过 OKR |
