...
```bash
mkdir -p "$WEEKLY_OUTPUT_DIR" "$MONTHLY_REVIEW_DIR" "$FEEDBACK_DIR"
```
...
先确定绩效月份。用户消息中已给出月份时直接使用；否则**默认取本月**（如当前 6 月，默认写 6 月绩效）。月度绩效通常在月末书写、总结当月，因此**不要在月末默认回退到上月**——例如 6 月 29 日调用默认写 6 月绩效，而非 5 月。仅当确实无法判断（如月初本月数据不足、跨月边界、用户意图不明）时，再通过 `AskUserQuestion` 询问用户在「本月 / 上月」中选择。
...
3. 读取 `$MONTHLY_REVIEW_DIR/*.md` 中最近 1-3 份历史月度绩效，作为表达风格、工作连续性和避免重复表述的参考。
4. 如果目标月份已经存在归档月度绩效，先询问用户是基于旧版修订、重新生成，还是忽略旧版。
5. 如果领导反馈文件不存在且用户也未提供反馈，"落实情况"部分跳过不写。

历史月度绩效只能作为风格和连续性参考；除非用户确认，否则不要把历史月份的工作当成本月实际产出。
...
保存到 `$MONTHLY_REVIEW_DIR/YYYY年MM月 月度绩效.md`。只保存用户确认后的最终版本，不保存中间草稿。
...
created_at: 2026-06-30T20:30:00+08:00
---

最终月度绩效正文...
```

如果目标文件已存在，询问用户"覆盖旧版本 / 另存为 `-v2` / 放弃保存"。
...
| `$WORK_REPORTER_HOME/monthly-reviews/*.md` | 已归档月度绩效 | `monthly-performance-writer` 用户确认后自动保存，也可由 `work-history-initializer` 导入 |
